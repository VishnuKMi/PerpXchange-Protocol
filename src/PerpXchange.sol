...
import {IPerpXchange} from "./IPerpXchange.sol";
...
contract PerpXchange is ERC4626, IPerpXchange, Ownable, ReentrancyGuard {
    struct Position {
        bool isLong;
        uint256 totalValue; // USD value of position
        uint256 size;
        uint256 collateral;
        address owner;
    ...
    // 20% of liquidity reserved for safety reasons
    ...
    uint256 private constant DECIMALS_PRECISION = 1e3; // to avoid truncation precision

    uint8 private liquidationDenominator = 10; // 10% of the collateral
    uint256 private maxLeverage = 2 * 1e4; // 20000
    ...
    constructor(address priceFeed, IERC20 _usdc) ERC4626(_usdc) ERC20("PerpX", "PX") Ownable(msg.sender) {
        i_priceFeed = AggregatorV3Interface(priceFeed);
        i_usdc = IERC20(_usdc);

        // Avoiding inflation attack by sending shares to contract
        _mint(address(this), DEAD_SHARES);
    }

    mapping(address => uint256) public collateral; // user to collateral
    mapping(uint256 => Position) public positions; // positionId to position
    mapping(address => EnumerableSet.UintSet) internal userToPositionIds; // user to positionIds
    mapping(address => int256) public userShortPnl; // user to short-pnl
    mapping(address => int256) public userLongPnl; // user to long-pnl

    /// External / Public Functions ///

    function depositCollateral(uint256 _amount) external nonReentrant {
        if (_amount < 0) revert PerpX__InvalidAmount();
    ...
    function withdrawCollateral() external {
        if (collateral[msg.sender] == 0) {
            revert PerpX__InsufficientCollateral();
        }
        if (userToPositionIds[msg.sender].length() > 0) {
            revert PerpX__OpenPositionExists();
        }
    ...
    function mint(uint256 shares, address receiver) public override nonReentrant returns (uint256 assets) {
        assets = super.mint(shares, receiver); // Interaction ?
        s_totalLiquidityDeposited += assets; // Effect ?
    }
    ...
    function closePosition(uint256 _positionId) public {
        ...
        if (position.owner != msg.sender) revert PerpX__NotOwner();
    ...
    function increaseCollateral(uint256 _positionId, uint256 _collateral) external nonReentrant {
        Position storage position = positions[_positionId];
        if (_collateral == 0) revert PerpX__InvalidCollateral();
        if (position.owner != msg.sender) revert PerpX__NotOwner();
    ...
    function liquidate(address _user) external nonReentrant {
        ...
        if (userLeverage <= maxLeverage) revert PerpX__NoLiquidationNeeded();
        ...
        internal
    {
        if (_isLong) {
            ...
            revert PerpX__NoPositionChosen();
        }
    }
    ...
    function _maxLiquidityUtilization(uint256 assets) internal view {
        ...
        if (s_shortOpenInterest + (s_longOpenInterestInTokens * currentPrice) > updatedLiquidity * DECIMALS_DELTA) {
            revert PerpX__InsufficientLiquidity();
        }
    }
    ...
    function getAverageOpenPrice(uint256 _positionId) public view returns (uint256) {
        ...
        if (_positionId == 0 || position.totalValue == 0 || position.size == 0) {
            revert PerpX__InvalidPositionId();
        }
    ...
    function maxRedeem(address owner) public view override returns (uint256 maxRedeemAllowed) {
        ...
        if (ownerAssets >= updatedLiquidity) {
            ...
            return maxRedeemAllowed = super._convertToShares(updatedLiquidity, Math.Rounding.Floor);
        }
